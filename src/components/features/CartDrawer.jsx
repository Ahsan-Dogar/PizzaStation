...
const CartDrawer = ({ isOpen, onClose, cart, updateQuantity, removeFromCart, clearCart, cartTotal }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end" role="dialog" aria-modal="true" aria-labelledby="cart-title">
      ...
      <div className="relative w-full sm:w-[400px] md:w-[450px] bg-[#1A1A1A] h-full shadow-2xl flex flex-col border-l border-gray-800 animate-in slide-in-from-right duration-300">
        
        <div className="bg-[#0F0F0F] p-5 flex justify-between items-center border-b border-gray-800">
          <h2 id="cart-title" className="text-xl font-black text-white flex items-center gap-2">
            <ShoppingBag className="text-[#FACC15]" /> 
            {step === 'cart' ? 'Aap Ka Cart' : step === 'payment' ? 'Payment Karein' : 'Order Confirm!'}
          </h2>
          <button aria-label="Close Cart" onClick={handleClose} className="text-gray-400 hover:text-white bg-gray-800/50 p-2 rounded-full">
            <X size={20} />
          </button>
        </div>
          ...
          {step === 'cart' && (
            cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-400">
                ...
                <p className="text-lg">Aap ka cart khali hai</p>
                <button onClick={handleClose} className="mt-4 text-[#F97316] font-bold">Menu dekhein</button>
              </div>
            ) : (
              <div className="space-y-4">
                {cart.map(item => (
                  <div key={item.id} className="flex gap-4 bg-[#0F0F0F] p-3 rounded-xl border border-gray-800">
                    <img src={item.image} alt={item.name} className="w-20 h-20 object-cover rounded-lg" />
                    <div className="flex-grow flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-start">
                          <h4 className="font-bold text-white leading-tight">{item.name}</h4>
                          <button aria-label="Remove item" onClick={() => removeFromCart(item.id)} className="text-red-500 hover:text-red-400 p-1">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <span className="font-black text-[#FACC15]">Rs. {item.price * item.quantity}</span>
                        <div className="flex items-center gap-3 bg-[#1A1A1A] rounded-lg px-2 py-1 border border-gray-800">
                          <button aria-label="Decrease quantity" onClick={() => updateQuantity(item.id, -1)} className="text-gray-400 hover:text-white p-1"><Minus size={14} /></button>
                          <span className="font-bold text-sm min-w-[12px] text-center">{item.quantity}</span>
                          <button aria-label="Increase quantity" onClick={() => updateQuantity(item.id, 1)} className="text-gray-400 hover:text-white p-1"><Plus size={14} /></button>
                        </div>
                      </div>
                    </div>
                  </div>
